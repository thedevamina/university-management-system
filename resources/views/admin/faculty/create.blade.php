<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Faculty</h2>
    </x-slot>

    <div class="py-6 max-w-lg mx-auto sm:px-6 lg:px-8">
        <form action="{{ route('admin.faculty.store') }}" method="POST" class="bg-white p-6 shadow rounded space-y-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="designation" value="{{ old('designation') }}" class="w-full border rounded p-2" placeholder="e.g. Lecturer, Professor">
                @error('designation') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block mb-1">Employee Number</label>
                <input type="text" name="employee_no" value="{{ old('employee_no') }}" class="w-full border rounded p-2">
